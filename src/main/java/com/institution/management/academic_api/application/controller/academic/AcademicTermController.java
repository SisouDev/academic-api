...
public class AcademicTermController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<AcademicTermDetailsDto> findById(@PathVariable Long id) {
        AcademicTermDetailsDto term = academicTermService.findById(id);
        return ResponseEntity.ok(term);
    }

    @GetMapping
    public ResponseEntity<List<AcademicTermSummaryDto>> findAllByInstitution(@RequestParam Long institutionId) {
